 ...
 * assumes that player knows how {@link app.service.strategy.ScientificMoveStrategy} works and will change to move on the
 * next turn so beat the move generated by @ScientificMoveStrategy.
 * For example, if player lost with combination (PAPER, ROCK) on the next turn
 * {@link app.service.strategy.ScientificMoveStrategy} will assume the player chooses SCISSORS, so generates ROCK.
 * But if player is aware of this strategy he will choose PAPER, so server should use SCISSORS.
 */
@Component
